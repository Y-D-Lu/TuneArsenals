...
import cn.arsenals.library.shell.FstrimUtils
import cn.arsenals.library.shell.ZenModeUtils
...
class TaskActionsExecutor(
    ...
    private fun updateNotification(text: String) {
        val notic = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (!channelCreated) {
                nm.createNotificationChannel(NotificationChannel("tunearsenals-task", context.getString(R.string.notice_channel_task), NotificationManager.IMPORTANCE_LOW))
                channelCreated = true
            }
            NotificationCompat.Builder(context, "tunearsenals-task")
        } else {
