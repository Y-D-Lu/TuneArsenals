...
class TuneArsenalsMode private constructor(private val context: AccessibilityTuneArsenals, private var store: TuneArsenalsConfigStore) {
     ...
     * 从应用离开时
     */
    fun onAppLeave(tunearsenalsConfigInfo: TuneArsenalsConfigInfo) {
        // 离开偏见应用时，记录偏见应用最后活动时间
        if (tunearsenalsConfigInfo.freeze) {
            setFreezeAppLeaveTime(tunearsenalsConfigInfo.packageName)
        }

        if (tunearsenalsConfigInfo.aloneLight) {
            // 独立亮度 记录最后的亮度值
            try {
                val light = Settings.System.getInt(contentResolver, Settings.System.SCREEN_BRIGHTNESS)
                if (light != tunearsenalsConfigInfo.aloneLightValue) {
                    tunearsenalsConfigInfo.aloneLightValue = light
                    store.setAppConfig(tunearsenalsConfigInfo)
                }
        ...
        if (tunearsenalsConfigInfo.fgCGroupMem != tunearsenalsConfigInfo.bgCGroupMem) {
                CGroupMemoryUtlis(TuneArsenals.context).run {
                    if (isSupported) {
                        if (tunearsenalsConfigInfo.bgCGroupMem?.isNotEmpty() == true) {
                            setGroupAutoDelay(this, tunearsenalsConfigInfo.packageName!!, tunearsenalsConfigInfo.bgCGroupMem)
                            // TuneArsenals.toast(tunearsenalsConfigInfo.packageName!! + "退出，cgroup调为[${tunearsenalsConfigInfo.bgCGroupMem}]\n(TuneArsenals试验性功能)")
                        } else {
                            setGroup(tunearsenalsConfigInfo.packageName!!, "")
                            // TuneArsenals.toast(tunearsenalsConfigInfo.packageName!! + "退出，cgroup调为[/]\n(TuneArsenals试验性功能)")
                        }
                    } else {
        ...
        // KeepShellPublic.doCmdSync("pm suspend ${tunearsenalsConfigInfo.packageName}")
    }
