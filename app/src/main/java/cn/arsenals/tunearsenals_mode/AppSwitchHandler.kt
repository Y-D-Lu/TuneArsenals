...
class AppSwitchHandler(private var context: AccessibilityTuneArsenals, override val isAsync: Boolean = false) : ModeSwitcher(), IEventReceiver {
    ...
    private var tunearsenalsBlackList = context.getSharedPreferences(SpfConfig.SCENE_BLACK_LIST, Context.MODE_PRIVATE)
    ...
    private val tunearsenalsMode = TuneArsenalsMode.getNewInstance(context, TuneArsenalsConfigStore(context))!!
    private var timer: Timer? = null
    private var tunearsenalsAppChanged: BroadcastReceiver? = null
                ...
                timer = Timer(true).apply {
                    ...
                    scheduleAtFixedRate(object : TimerTask() {
                        ...
                        override fun run() {
                            ...
                            ticks %= 60
                            if (ticks == 0) {
                                tunearsenalsMode.clearFreezeAppTimeLimit()
                            }
                        }
                    }, 0, interval * 1000L)
                }
    ...
    private fun onScreenOffCloseNetwork() {
        if (!screenOn) {
            if (System.currentTimeMillis() - lastScreenOnOff >= SCREEN_OFF_SWITCH_NETWORK_DELAY) {
                tunearsenalsMode.onScreenOffDelay()
                System.gc()
            }
        }
    }
    ...
    private fun onScreenOn() {
        ...
        handler.postDelayed({
            if (dynamicCore && lastMode.isNotEmpty()) {
                ...
                tunearsenalsMode.cancelFreezeAppThread()
            }
        }, 1000)
        tunearsenalsMode.onScreenOn()
            ...
            EventType.SCENE_APP_CONFIG -> {
                data?.run {
                    if (containsKey("app")) {
                        ...
                        tunearsenalsMode.updateAppConfig()
                    }
                }
            }
            else -> return
    ...
    override fun onUnsubscribe() {
        tunearsenalsMode.clearState()
        notifyHelper.hideNotify()
        stopTimer()
        if (tunearsenalsAppChanged != null) {
            context.unregisterReceiver(tunearsenalsAppChanged)
            tunearsenalsAppChanged = null
        }
    ...
    private fun onFocusedAppChanged(packageName: String) {
        ...
        if (lastPackage == packageName || ignoredList.contains(packageName) || tunearsenalsBlackList.contains(packageName)) return
        if (lastPackage == null) lastPackage = "com.android.systemui"

        autoToggleMode(packageName)
        tunearsenalsMode.onAppEnter(packageName)
